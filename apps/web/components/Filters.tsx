...
const EMPTY_OPTIONS: DynamicOptions = {
  product_types: [], styles: [], materials: [], colors: [], patterns: [], seasons: [],
...
export default function Filters({ filters, setFilter, clearAll }: Props): React.ReactElement {
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      ...
      setOptions({
        product_types:     unique(data.map((img: any) => img.ai_metadata?.product_type)),
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm overflow-y-auto max-h-[calc(100vh-6rem)]">
      ...
      <div className="px-4 py-3 space-y-4">
        <ChipGroup label="Product Type"     filterKey="product_type"     values={options.product_types} />
