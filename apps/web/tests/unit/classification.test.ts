...
describe('parseClassification', () => {
  it('parses a valid JSON string into a typed Classification', () => {
    ...
    expect(parsed.metadata.product_type).toBe('shirt')
  ...
  it('accepts a pre-parsed object (not a string)', () => {
    const obj = { description: 'A dress', metadata: { ...VALID_METADATA, product_type: 'dress' } }
    const parsed = parseClassification(obj)
    expect(parsed.metadata.product_type).toBe('dress')
  })
  ...
  it('returns all required metadata fields', () => {
    const parsed = parseClassification(VALID_INPUT)
    const required = [
      'product_type', 'style', 'material', 'color_palette', 'pattern',
