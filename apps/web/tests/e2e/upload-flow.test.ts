...
test.describe('Dashboard – filter and search', () => {
  ...
  test('product type filter chip triggers fetch with product_type param', async ({ page }) => {
    ...
    // After options load, click the product type chip labelled "dress".
    const chip = page.locator('[data-testid="filter-chip"]', { hasText: 'dress' })
    if (await chip.count() > 0) {
      await chip.first().click()
      await page.waitForTimeout(300)
      expect(filterURL).toContain('product_type=dress')
    }
  })
})
