...
function parseParams(sp: URLSearchParams) {
  return {
    q:               sp.get('q')?.trim()  ?? '',
    productType:     sp.get('product_type') || sp.get('type') || '',
    style:           sp.get('style') ?? '',
...
describe('Search API – query string parsing', () => {
  it('parses product_type filter', () => {
    const p = parseParams(buildSearchParams({ product_type: 'shirt' }))
    expect(p.productType).toBe('shirt')
  })

  it('accepts legacy "type" alias for product_type', () => {
    const p = parseParams(buildSearchParams({ type: 'dress' }))
    expect(p.productType).toBe('dress')
  })
  ...
  it('combines location + product + text filters simultaneously', () => {
    const p = parseParams(
      buildSearchParams({ city: 'Tokyo', product_type: 'dress', q: 'floral' })
    )
